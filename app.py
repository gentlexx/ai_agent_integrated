...
def chat_with_process(message, history):
    """带思考过程的聊天"""
    if not message:
        return "", history

    #调用带过程的方法
    result = mas.run_with_process(message)
...
def get_memory_status():
    """获取记忆状态"""
    count = len(mas.conversation_history)
...
with gr.Blocks(title="多智能体协作系统") as demo:
    ...
    **规划师 → 研究员 → 撰写员**
    ...
    # 方式1：直接绑定 Textbox 的 submit 事件
    msg.submit(respond, [msg, chatbot], [msg, chatbot])

    clear.click(lambda: [], None, chatbot, queue=False)
    clear_memory_btn.click(clear_all, None, chatbot, queue=False)
